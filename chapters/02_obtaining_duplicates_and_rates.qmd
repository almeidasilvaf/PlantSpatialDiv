...
Then, we will use Ks values to split gene pairs by age groups, so that
...
2. *Zea mays*: 2 peaks at $K_s$ ~0.16 and 0.9. The second peak cannot be 
distinguished from the $K_s$ distro of *Zea mays* because anchor pairs 
originating from this ancient WGD event cannot be detected anymore. Hence,
we calculated the mean of $K_s$ peaks estimates for all species in which
this WGD event could be detected (N = 11), using data from 
AngioWGD [@chen2024rise]. Following this approach, we will consider a peak at
0.9 (0.51-1.89).
3. *Arabidopsis thaliana*: 2 peaks at $K_s$ ~0.73 and 1.61. The second peak
cannot be distinguished from the $K_s$ distro of *Arabidopsis thaliana* because
peaks largely overlap. To distinguish between the two peaks, we used prior
data from [@chen2024rise], which identified the second peak more clearly
(1.61, 0.90-3.00) in the $K_s$ distro of *Cleome violacea*.
...
Finally, we will classify SD pairs in WGD peaks as 'WGD'.

```{r}
# Relabel SD pairs in peaks as 'WGD'
ath_wgd <- pairs_byage$ath |> filter(type == "SD")
ath_wgd <- unique(c(ath_wgd$dup1, ath_wgd$dup1))
ath_dups <- ath_dups |> mutate(type = ifelse(gene %in% ath_wgd, "WGD", type))

gma_wgd <- pairs_byage$gma |> filter(type == "SD")
gma_wgd <- unique(c(gma_wgd$dup1, gma_wgd$dup1))
gma_dups <- gma_dups |> mutate(type = ifelse(gene %in% gma_wgd, "WGD", type))

pap_wgd <- pairs_byage$pap |> filter(type == "SD")
pap_wgd <- unique(c(pap_wgd$dup1, pap_wgd$dup1))
pap_dups <- pap_dups |> mutate(
    type = as.character(type),
    type = ifelse(gene %in% pap_wgd, "WGD", type)
)

zma_wgd <- pairs_byage$zma |> filter(type == "SD")
zma_wgd <- unique(c(zma_wgd$dup1, zma_wgd$dup1))
zma_dups <- zma_dups |> mutate(type = ifelse(gene %in% zma_wgd, "WGD", type))

hvu_wgd <- pairs_byage$hvu |> filter(type == "SD")
hvu_wgd <- unique(c(hvu_wgd$dup1, hvu_wgd$dup1))
hvu_dups <- hvu_dups |> mutate(type = ifelse(gene %in% hvu_wgd, "WGD", type))
...
saveRDS(
    ...
    file = here("products", "result_files", "dup_list_expanded.rds")
)
