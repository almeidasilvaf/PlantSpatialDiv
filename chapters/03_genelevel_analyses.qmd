...
dup_list <- readRDS(
    here("products", "result_files", "dup_list_expanded.rds")
)
...
sum_all <- Reduce(rbind, lapply(names(spe_all), function(x) {
    ...
    mutate(type = factor(type, levels = names(dup_pal)))
...
The figures show that, overall, WGD, segmental, tandem, and proximal duplicates 
...
Interestingly, SVGs are enriched in large-scale duplication 
(SD and WGD)-derived genes in most of the samples,
