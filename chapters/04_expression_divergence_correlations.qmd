...
a simple linear regression, indicating that the two variables (Ks and 
...
However, a positive slope ($\beta_1$) can be observed for retrotransposed
...
this duplication mode. It is noteworthy that the number of retrotransposed
...
## List with coexpression networks (no edge lists) for each sample-species
saveRDS(
    gcns, compress = "xz",
    file = here("products", "result_files", "gcns.rds")
)
